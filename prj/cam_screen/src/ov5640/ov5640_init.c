...
u8 sccb_read_reg16(u16 addr )
{
	...
	sccb_send_byte(SCCB_ID | 0x01);  //SCCB读数据
	sccb_ack();
...
u8 ov5640_init( u16 cmos_h_pixel,  u16 cmos_v_pixel,  
				u16 total_h_pixel,  u16 total_v_pixel )
{
	u16 cam_id = 0;
...
//		sccb_write_reg16(0x3036,0x3c); //PLL倍频  //48M
		...
		//时序控制 0x3800~0x3821
		sccb_write_reg16(0x3820,0x46); //上下翻转40/46
		sccb_write_reg16(0x3821,0x01); //左右镜像01/07
		...
		//设置输出像素个数
		sccb_write_reg16(0x3808, cmos_h_pixel >> 8     );  //DVP 输出水平像素点数高4位//8
		sccb_write_reg16(0x3809, cmos_h_pixel & 0x00FF );  //DVP 输出水平像素点数低8位//0FF
		sccb_write_reg16(0x380a, cmos_v_pixel >> 8     );  //DVP 输出垂直像素点数高3位//8
		sccb_write_reg16(0x380b, cmos_v_pixel & 0x00FF );  //DVP 输出垂直像素点数低8位//0FF
		...
		sccb_write_reg16(0x301c,0x02); //闪光灯IO选择
